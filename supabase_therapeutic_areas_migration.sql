-- =====================================================
-- Therapeutic Areas Migration for Supabase
-- Updates to top 15 common disease areas plus Others
-- =====================================================

-- Step 1: Check current therapeutic areas distribution
...
-- Step 2: Update existing values to match our new standard list
UPDATE submissions 
...
-- Step 3: Create an enum type for therapeutic areas
DO $$ 
BEGIN
...
-- Step 4: Alter the submissions table to use the enum type
ALTER TABLE submissions 
ALTER COLUMN therapeutic_area TYPE therapeutic_area_type 
USING therapeutic_area::therapeutic_area_type;

-- Step 5: Update projects table if it has therapeutic_area column
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 
...
-- Step 6: Add a comment to document the enum values
COMMENT ON TYPE therapeutic_area_type IS 'Enum type for therapeutic areas: Oncology, Cardiology, Neurology, Psychiatry, Endocrinology, Rheumatology, Dermatology, Pulmonology, Gastroenterology, Infectious Diseases, Immunology, Ophthalmology, Nephrology, Hematology, Gynecology, Others';
...
-- Optional: Create a helper function to validate therapeutic areas
CREATE OR REPLACE FUNCTION is_valid_therapeutic_area(area text) 
RETURNS boolean AS $$
BEGIN
    RETURN area::text IN (
        'Oncology', 'Cardiology', 'Neurology', 'Psychiatry', 
        'Endocrinology', 'Rheumatology', 'Dermatology', 'Pulmonology', 
        'Gastroenterology', 'Infectious Diseases', 'Immunology', 
        'Ophthalmology', 'Nephrology', 'Hematology', 'Gynecology', 'Others'
    );
END;
$$ LANGUAGE plpgsql;